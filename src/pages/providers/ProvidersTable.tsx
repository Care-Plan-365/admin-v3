        ...
        <Table>
          <thead className="bg-cp365-bg text-left text-xs font-semibold uppercase tracking-wide text-cp365-textMuted">
            <tr>
              ...
              {includePractice && <th className="px-6 py-4">Practice</th>}
              {includeMedicareMinor && (
                <th className="px-6 py-4">Medicare Location / Minor ID</th>
              )}
              {hasActions && <th className="px-6 py-4 text-right">Action</th>}
            </tr>
          </thead>
          <tbody>
            {providers.map((provider) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4">{getAHPRA(providerLike)}</td>
                  <td className="px-6 py-4">{provider.providerType}</td>
                  {includePractice && <td className="px-6 py-4">{getPractice(providerLike)}</td>}
                  {includeMedicareMinor && (
                  ...
                  {hasActions && (
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">{renderActions(provider)}</div>
                    </td>
                  )}
                </tr>
              );
            ...
            {providers.length === 0 && (
              <tr>
                <td
                  colSpan={4 + (includePractice ? 1 : 0) + (includeMedicareMinor ? 1 : 0) + (hasActions ? 1 : 0)}
