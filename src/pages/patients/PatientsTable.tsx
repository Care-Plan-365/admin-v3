...
const getInitial = (value: string) => (value ? value.charAt(0).toUpperCase() : '-');
...
export const PatientsTable = ({ patients, renderActions }: PatientsTableProps) => (
      ...
      <table className="w-full border-collapse text-sm text-cp365-textMain">
        <thead className="bg-cp365-bg text-left text-[11px] font-semibold uppercase tracking-[0.14em] text-cp365-textMuted">
          <tr>
            ...
            {renderActions && <th className="px-6 py-3 text-right">Action</th>}
          </tr>
        </thead>
        <tbody>
          {patients.map((patient) => (
            ...
            >
              <td className="px-6 py-4 font-semibold">{getInitial(patient.firstName)}</td>
              <td className="px-6 py-4 font-semibold">{getInitial(patient.lastName)}</td>
              <td className="px-6 py-4">{patient.suburb}</td>
              <td className="px-6 py-4">{patient.providerName}</td>
              {renderActions && (
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">{renderActions(patient)}</div>
                </td>
              )}
            </tr>
          ))}
